...
class TestResolveQuestion:
    def test_explicit_string(self):
        ctx = ChatContext()
        text, returned_ctx, docs = _resolve_question("hello", ctx)
        assert text == "hello"
    ...
    def test_from_context(self):
        ...
        text, rewound, docs = _resolve_question(None, ctx)
    ...
    def test_from_component_uses_backend_formatter(self):
        ...
        text, rewound, docs = _resolve_question(None, ctx, backend=mock_backend)
        assert text == "custom formatted"
        mock_backend.formatter.print.assert_called_once()
        assert rewound.is_root_node  # type: ignore[union-attr]
        assert docs is None

    def test_from_message_with_documents(self):
        doc = Document(text="Supporting text", doc_id="1")
        ctx = ChatContext().add(Message("user", "What is 2+2?", documents=[doc]))
        text, rewound, docs = _resolve_question(None, ctx)
        assert text == "What is 2+2?"
        assert docs is not None
        assert len(docs) == 1
        assert docs[0].text == "Supporting text"
        assert rewound.is_root_node  # type: ignore[union-attr]
...
class TestResolveResponse:
    def test_explicit_string(self):
        ctx = ChatContext()
        text, returned_ctx, docs = _resolve_response("answer", ctx)
        assert text == "answer"
        assert returned_ctx is ctx
        assert docs is None
    ...
    def test_from_context(self):
        ...
        text, rewound, docs = _resolve_response(None, ctx)
    ...
    def test_from_message_with_documents(self):
        ...
        ctx = (
            ChatContext()
            .add(Message("user", "question"))
            .add(Message("assistant", "The answer is 4.", documents=[doc]))
        )
        text, rewound, docs = _resolve_response(None, ctx)
        assert text == "The answer is 4."
        assert docs is not None
