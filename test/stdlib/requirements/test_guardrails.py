...
def test_no_pii_auto_mode_clean_text():
    ...
    if not result.as_bool():
        ...
        assert "PERSON" not in result.reason and "GPE" not in result.reason and "LOC" not in result.reason, \
...
def test_no_pii_has_description():
    ...
    assert "PII" in req.description or "personally identifiable" in req.description.lower()
...
def test_factual_grounding_high_overlap():
    ...
    ctx = create_context("Python is a programming language created by Guido van Rossum.")
    result = req.validation_fn(ctx)
...
def test_factual_grounding_low_overlap():
    ...
    ctx = create_context("JavaScript is used for web development with React and Node.js.")
    result = req.validation_fn(ctx)
