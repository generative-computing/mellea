...
def test_prepare_activate_deactivate_release_full_lifecycle():
    ...
    starts = invocation_start_payloads(mock_invoke)
    assert len(starts) == 1
    invocation_id = starts[0].adapter_function_invocation_id
    assert invocations[0].adapter_function_invocation_id == invocation_id
    assert all(
        p.adapter_function_invocation_id == invocation_id
        for p in [*phase_start_payloads(mock_invoke), *phase_payloads(mock_invoke)]
    )
