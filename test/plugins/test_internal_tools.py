...
from mellea.core.base import GenerateLog, ModelOutputThunk, ModelToolCall
...
async def test_tool_is_internal_accessible_in_hook():
    ...
    internal_tool = MelleaTool.from_callable(
        lambda: "ok", name="internal", is_internal=True
    )
    external_tool = MelleaTool.from_callable(
        lambda: "ok", name="external", is_internal=False
    )
    ...
    mot = ModelOutputThunk(
        value="", tool_calls={"internal": tc_internal, "external": tc_external}
    )
