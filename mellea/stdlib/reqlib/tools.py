...
def uses_tool(tool_name: str | Callable, check_only=False):
    """Forces the model to call a givne tool.
...
def tool_arg_validator(
        description: str, 
        tool_name: Optional[str | Callable], 
        arg_name: str, 
    ...
    def _validate(ctx: Context):
        ...
        if tool_name:
            if tool_name not in output.tool_calls:
                return ValidationResult(result=False, reason=f"Tool {tool_name} was not called.")
            if arg_name not in output.tool_calls[tool_name].args:
                return ValidationResult(result=False, reason=f"Tool {tool_name} did not call argument {arg_name}")
