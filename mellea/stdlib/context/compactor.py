...
from mellea.core import CBlock, Context, ModelOutputThunk, NodeData
...
PinPredicate: TypeAlias = Callable[[list[NodeData]], int]
...
def pin_nothing(components: list[NodeData]) -> int:
...
def pin_system(components: list[NodeData]) -> int:
...
def pin_system_and_initial_user(components: list[NodeData]) -> int:
...
class LLMSummarizeCompactor:
    ...
    async def _async_compact(
        self, ctx: ChatContext, backend: Backend, full: list[NodeData], pin_end: int
    ) -> ChatContext:
