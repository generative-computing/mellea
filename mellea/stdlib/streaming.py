...
class EventStreamer:
    """Async-iterable handle over a stream's `StreamEvent` objects.

    Returned by `stream(..., as_events=True)`; iterate with `async for` inside
    `async with`. Outcome attributes (`full_text`, `completed_normally`, …)
    mirror the wrapped `Streamer`. Do not instantiate directly.
    """
    ...
    def __aiter__(self) -> AsyncIterator[StreamEvent]:
        """Return the async generator draining events from the queue."""
        return self._drain()

    async def _drain(self) -> AsyncGenerator[StreamEvent, None]:
        """Yield queued events until the sentinel, re-raising any pump failure."""
        while True:
...
async def stream(
    ...
    *,
    ...
    only via the `STREAMING_EVENT` hook). Unlike chunk iteration, the terminal
    `CompletedEvent`/`ErrorEvent` are delivered on every exit path, including an
    early `break`. Each `ChunkEvent` carries its text, so no output is lost.
