...
class UnsafeEnvironment(ExecutionEnvironment):
    ...
    def __init__(
        ...
        # Callers may pass a shared set to persist the install cache across multiple
        # execute() calls (e.g. from the python_tool closure).
        self._installed_packages: set[str] = (
            _installed_packages if _installed_packages is not None else set()
        )
        # Explicit tier name so _mode() reports the tier that was requested rather
        # than inferring it from policy presence (which mislabels local_unsafe+packages).
        self._tier: str | None = _tier
    ...
    def execute(self, code: str, timeout: int | None = None) -> ExecutionResult:
        ...
        if self.policy and self.policy.packages:
            pending = [
                p for p in self.policy.packages if p not in self._installed_packages
            ]
            if pending:
...
class LLMSandboxEnvironment(ExecutionEnvironment):
    ...
    def _install_packages_in_session(self, session: Any) -> None:
        ...
        if not (self.policy and self.policy.packages):
            return
        pending = [p for p in self.policy.packages if p not in self._installed_packages]
        if not pending:
            return
    ...
    def execute(self, code: str, timeout: int | None = None) -> ExecutionResult:
        ...
        try:
            if self._session is not None:
                ...
                self._install_packages_in_session(self._session)
                ...
                with SandboxSession(
                    ...
                    try:
                        self._install_packages_in_session(session)
                    finally:
    ...
    match tier:
        ...
        case "local_unsafe":
            ...
            return UnsafeEnvironment(
                ...
                working_directory=working_directory,
                _installed_packages=_install_cache,
                _tier="local_unsafe",
            )
        case "local":
...
def python_tool(
    ...
    The returned :class:`~mellea.backends.tools.MelleaTool` wraps a callable with
    signature ``run_python(code: str) -> ExecutionResult``.  It can be passed
    directly to ``ModelOption.TOOLS`` in agentic ReACT loops.

    For **local tiers** (``"local_unsafe"``, ``"local"``), files written to
    ``artifact_dir`` (or to the per-call tempdir when ``artifact_dir`` is
    ``None``) are surfaced as :class:`~mellea.stdlib.tools.Artifact` objects
    on the returned :class:`~mellea.stdlib.tools.ExecutionResult`.  Only files
    produced by a **successful** execution (exit code 0) are included.

    .. note::
        **Docker tiers** (``"docker"``, ``"docker_unsafe"``) do not support
        artifact scanning.  ``artifact_dir`` is ignored for these tiers and
        ``result.artifacts`` is always ``[]``.  A :class:`RuntimeWarning` is
        emitted if ``artifact_dir`` is passed with a docker tier.
    ...
    Args:
        ...
        packages (list[str] | None): Python packages to pre-install via
            ...
            with ``-`` (flag-style arguments are rejected).
        artifact_dir (Path | None): Directory where the executed code should
            ...
            :class:`~mellea.stdlib.tools.ExecutionResult` holds artifacts, and
            cleaned up immediately when no artifacts are produced.  Ignored for
            docker tiers.
        policy (CapabilityPolicy | None): Override the tier's default
            :class:`~mellea.stdlib.tools.CapabilityPolicy`.  When ``packages``
            is also provided, those packages are merged into this policy.
        allowed_imports (list[str] | None): Allowlist of importable top-level
    ...
    # Shared install cache — persists across all run_python() calls on this tool
    # instance so packages are only installed once per tool lifetime, not per call.
    ...
    def run_python(code: str) -> ExecutionResult:
        ...
        # No caller-supplied artifact_dir.  Create a per-call tempdir and clean
        # it up only when execution produced no artifacts — if artifacts were
        # written we keep the directory alive so the caller can read the files.
