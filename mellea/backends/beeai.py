...
class BeeAIBackend(FormatterBackend):
    ...
    def __init__(
        ...
        """
        super().__init__(
            model_id=model_id, formatter=formatter, model_options=model_options or {}
        )
        ...
        if model_id.startswith(("granite", "llama", "mistral")):
    ...
    def _convert_context_to_messages(
        ...
        if hasattr(ctx, "render_for_generation") and callable(
        ...
        *,
        ...
        try:
            ...
            response = chat_model.create(
                messages=messages, **beeai_params.model_dump(exclude_none=True)
            )
            ...
            if (
                ...
                or hasattr(response, "_mock_return_value")
                or str(type(response)).find("Mock") != -1
            ):
                ...
                if hasattr(response, "messages") and response.messages:
                    ...
                    if hasattr(first_message, "content"):
                        if isinstance(first_message.content, list):
                            ...
                            content = " ".join(
                                [str(block) for block in first_message.content]
                            )
                        else:
                            content = str(first_message.content)
                    else:
        ...
        *,
        ...
        try:
            ...
            for action in actions:
                ...
                chat_model = self._get_chat_model()
                response = chat_model.create(
                    messages=[message], **beeai_params.model_dump(exclude_none=True)
                )
            ...
            try:
                if self.provider == "ollama":
                    ...
                    self._chat_model = ChatModel.from_name(
                        "ollama", model=self.model_id, **self._chat_kwargs
                    )
                else:
                    self._chat_model = ChatModel.from_name(
                        self.provider, **self._chat_kwargs
                    )
            except Exception as e:
                FancyLogger.get_logger().warning(
                ...
                self.provider = "openai"
                self._chat_model = ChatModel.from_name("openai", **self._chat_kwargs)
        return self._chat_model

    def _merge_model_options(
        self, call_options: dict[str, Any] | None
    ) -> dict[str, Any]:
