...
class BeeAITrace:
    ...
    def finish(self, output_data: Optional[Dict[str, Any]] = None):
...
class BeeAITraceContext:
    ...
    def __init__(self):
        ...
        self.current_trace: Optional[BeeAITrace] = None
    ...
    def end_trace(self, output_data: Optional[Dict[str, Any]] = None):
...
class BeeAIPlatformBackend(BeeAIBackend):
    ...
    def _should_trace(self, level: str) -> bool:
        ...
        if self.trace_granularity == "all":
            return True
        elif self.trace_granularity == "component" and level in ["component", "generate"]:
            return True
    ...
    def generate_from_context(
        ...
        *,
        ...
        if self._should_trace("generate"):
            input_data = {
                "action": str(action),
                "context_length": len(ctx.render_for_generation()) if hasattr(ctx, 'render_for_generation') else 0,
                "model_options": model_options or {},
        ...
        try:
            ...
            if trace:
                output_data = {
                    ...
                    "parsed_repr": str(result.parsed_repr) if result.parsed_repr else None,
                    "tool_calls": list(result.tool_calls.keys()) if result.tool_calls else [],
                }
...
def start_beeai_platform(
    ...
    if background:
        ...
        FancyLogger.get_logger().info(f"BeeAI platform started in background (PID: {process.pid})")
        return process
    else:
        subprocess.run(cmd)
        return None
