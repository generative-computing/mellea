...
sidebar_label: "mellea.stdlib.sampling.budget_forcing"
...
### <span className="ml-2 inline-flex items-center rounded-full px-2 py-1 text-[0.7rem] font-bold tracking-wide bg-[#4ADE8033]/20 text-[#15803D]">CLASS</span> `BudgetForcingSamplingStrategy` <sup><a href="https://github.com/generative-computing/mellea/blob/v0.8.0/mellea/stdlib/sampling/budget_forcing.py#L31" target="_blank"><Icon icon="github" style="width: 0.85em; height: 0.85em;" /></a></sup> {#class-budgetforcingsamplingstrategy}


Sampling strategy that enforces a token budget for chain-of-thought reasoning.

Extends [`RejectionSamplingStrategy`](base#class-rejectionsamplingstrategy) with explicit control over the `<think>`
