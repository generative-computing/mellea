...
### <span className="ml-2 inline-flex items-center rounded-full px-2 py-1 text-[0.7rem] font-bold tracking-wide bg-[#3064E3]/20 text-[#1D4ED8]">FUNC</span> `think_budget_forcing` <sup><a href="https://github.com/generative-computing/mellea/blob/v0.8.0/mellea/stdlib/sampling/sampling_algos/budget_forcing_alg.py#L34" target="_blank"><Icon icon="github" style="width: 0.85em; height: 0.85em;" /></a></sup> {#think_budget_forcing}

```python
think_budget_forcing(backend: OllamaModelBackend, action: Span) -> ModelOutputThunk
```
