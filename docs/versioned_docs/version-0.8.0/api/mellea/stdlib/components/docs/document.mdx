...
typically attached to a [`Message`](../chat#class-message) via its `documents` parameter, enabling
...
### <span className="ml-2 inline-flex items-center rounded-full px-2 py-1 text-[0.7rem] font-bold tracking-wide bg-[#4ADE8033]/20 text-[#15803D]">CLASS</span> `Document` <sup><a href="https://github.com/generative-computing/mellea/blob/v0.8.0/mellea/stdlib/components/docs/document.py#L18" target="_blank"><Icon icon="github" style="width: 0.85em; height: 0.85em;" /></a></sup> {#class-document}


A text passage with optional metadata for grounding model inputs.

Documents are typically attached to a [`Message`](../chat#class-message) via its `documents`
