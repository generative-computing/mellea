...
#### <span className="ml-2 inline-flex items-center rounded-full px-2 py-1 text-[0.7rem] font-bold tracking-wide bg-[#3064E3]/20 text-[#1D4ED8]">FUNC</span> `to_chat_messages` <sup><a href="https://github.com/generative-computing/mellea/blob/v0.7.0/mellea/formatters/chat_formatter.py#L24" target="_blank"><Icon icon="github" style="width: 0.85em; height: 0.85em;" /></a></sup> {#to_chat_messages}

```python
to_chat_messages(self, cs: list[Component | CBlock | ModelOutputThunk]) -> list[Message]
```
