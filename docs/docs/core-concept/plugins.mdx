...
A hook returns either `None` (continue unchanged) or a `PluginResult` (to modify the payload or block execution). The quickstart hook returns `None` implicitly — it observes without interfering.
...
**`plugin_scope()` — the universal scope:**
...
**Writable fields:** `backend_name`, `model_id`, `model_options`

**Use cases:**
- Routing to alternative backends based on policy
...
**Payload fields:** `component_type`, `action`, `context`, `context_view`, `requirements`, `model_options`, `format`, `strategy`, `tool_calls_enabled`

**Writable fields:** `action`, `context`, `context_view`, `requirements`, `model_options`, `format`, `strategy`, `tool_calls_enabled`
...
**Writable fields:** `repair_action`, `repair_context`
...
All hooks for a given hook type are sorted by priority, then dispatched in groups by execution mode. The execution order is always:
...
<Note>
The FIRE_AND_FORGET log output may appear after the main result is printed. This is expected behavior — these hooks run in the background.
</Note>
...
- **`SEQUENTIAL`, `TRANSFORM`, `AUDIT`, `CONCURRENT`** — unexpected exceptions propagate to the caller.
- **`FIRE_AND_FORGET`** — exceptions are logged and swallowed. They never affect the pipeline.
- **`block()`** — this is intentional control flow, not an error. It raises `PluginViolationError` in the caller.
...
The `tool_pre_invoke` and `tool_post_invoke` hooks give you fine-grained control over tool-call governance. See [Chapter 11 of the tutorial](/core-concept/interoperability) for tool calling basics.
...
Redact PII on input (`component_pre_execute`) and output (`generation_post_call`) using a class-based plugin with shared redaction state:
...
Deploy a new policy in `AUDIT` mode first — violations are logged but do not block. Monitor the logs. When you're confident, promote to `SEQUENTIAL`:
...
If you use the advanced `MelleaPlugin` base class (which provides `initialize()` and `shutdown()` callbacks), make them idempotent — they may be called once per `@hook` method on your plugin.
