...
The orchestrator emits typed events throughout execution. Use `result.events()`
in place of, or alongside, `result.astream()`:
...
async for event in result.events():
    match event:
        ...
        case QuickCheckEvent(passed=False):
            print(f"validation failed at chunk {event.chunk_index}")
