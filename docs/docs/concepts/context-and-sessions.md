...
Every call to an LLM in Mellea passes through four layers: [**Component**](../reference/glossary.md#component), [**Backend**](../reference/glossary.md#backend),
[**Context**](../reference/glossary.md#context), and **Session**. Understanding how these fit together explains both why
Mellea is structured the way it is and how to extend it effectively.

> **Looking to use this in code?** See [Context and Sessions](../how-to/use-context-and-sessions.md) for practical examples and session extension patterns.
...
returns the model output as a [`ModelOutputThunk`](../reference/glossary.md#modeloutputthunk). The `Thunk` is a lazy wrapper: it
...
1. **Prompt construction** — the backend calls `ctx.view_for_generation()` to get
   ...
   all prior turns. For [`SimpleContext`](../reference/glossary.md#simplecontext), it includes only the current instruction.
...
[`MelleaSession`](../reference/glossary.md#melleasession) is the developer-facing layer. It wraps a backend and a context,
...
`last_turn()` returns a [`ContextTurn`](../reference/glossary.md#contextturn) with `.input` and `.output` fields. It is
