...
- **[Component](../reference/glossary.md#component) Lifecycle** — before and after component execution (`component_pre_execute`, `component_post_success`, `component_post_error`)
- **Generation Pipeline** — before and after LLM calls (`generation_pre_call`, `generation_post_call`, `generation_error`)
- **Validation** — before and after [requirement](../reference/glossary.md#requirement) checks (`validation_pre_check`, `validation_post_check`)
...
Pass plugins to [`start_session()`](../reference/glossary.md#melleasession), fires only within that session.
...
**Payload fields:** `sampling_id`, `strategy_name`, `iteration`, `sample_index`, `action`, `result`, `validation_results`, `all_validations_passed`, `valid_count`, `total_count`
...
**Payload fields:** `sampling_id`, `repair_type`, `failed_action`, `failed_result`, `failed_validations`, `repair_action`, `repair_context`, `repair_iteration`, `sample_index`
...
The `tool_pre_invoke` and `tool_post_invoke` hooks give you fine-grained control over tool-call governance. See the [MCP integration guide](../integrations/mcp.md) for tool calling basics.
...
**See also:** [Glossary](../reference/glossary.md), [Tools and Agents](../how-to/tools-and-agents.md), [Safety Guardrails](../how-to/safety-guardrails.md), [OpenTelemetry Tracing](../observability/tracing.md)
