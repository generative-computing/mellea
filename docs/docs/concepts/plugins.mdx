...
:::note
**Creation hooks** (`component_pre_create` / `component_post_create`) are not
implemented. `Component` is a `Protocol`, not an abstract base class, so there is no
single interception point that covers all component implementations. Use
`component_pre_execute` for pre-execution policy enforcement instead.
:::
