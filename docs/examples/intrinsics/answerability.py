...
result = rag.check_answerability(
    "What is the square root of 4?", documents=["The square root of 4 is 2."], context=ctx, backend=backend
)
print(f"Result of answerability check when answer is in documents: {result}")

result = rag.check_answerability(
    "What is the square root of 4?", documents=["The square root of 8 is not 2."], context=ctx, backend=backend
)
