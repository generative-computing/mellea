...
def example_length_constraints():
    ...
    # Maximum length
    result = m.instruct(
        "Write a one-sentence summary of Python", requirements=[max_length(100)]
    )
    print(
        f"Short summary ({len(result.value) if result.value else 0} chars): {result.value}"
    )

    # Minimum length
    result = m.instruct(
        "Write a detailed explanation of REST APIs", requirements=[min_length(200)]
    )
    print(
        f"Detailed explanation ({len(result.value) if result.value else 0} chars): {result.value[:100] if result.value else 'None'}..."
    )

    # Word-based constraints
    result = m.instruct(
        "List 5 programming languages", requirements=[max_length(50, unit="words")]
    )
...
def example_keyword_matching():
    ...
    result = m.instruct(
        ...
        requirements=[
            contains_keywords(["HTTP", "JSON", "endpoint"], require_all=True)
        ],
    )
...
def example_case_sensitivity():
    ...
    print("Case-insensitive match: Success")
    ...
    print("Case-sensitive match: Success")
...
def example_harmful_content_detection():
    ...
    result = m.instruct(
        "Write a helpful guide about online safety", requirements=[no_harmful_content()]
    )
...
def example_combined_advanced():
    ...
    result = m.instruct(
        ...
        requirements=[matches_schema(code_schema), no_harmful_content()],
    )
    print(f"Combined validation result: {result.value}")
