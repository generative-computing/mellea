...
def chat_with_beeai_platform(
    message: str,
    model_id: str = "granite3.3-8b-instruct",
    ...
    system_message = Message(
        role="system",
        content="You are a helpful AI assistant created with Mellea and BeeAI Platform.",
    )
    ...
    result = session.backend.generate_from_context(
        action=user_message,
        ctx=session.ctx,
        model_options=model_options or {"temperature": 0.7, "max_tokens": 512},
    )
...
def serve(
    ...
    # Generate response
    response = chat_with_beeai_platform(message, model_options=model_options)
...
def chat_demo():
    ...
    while True:
        try:
            ...
            # Generate response
            response = chat_with_beeai_platform(user_input, session=session)
...
def main():
    ...
    if len(sys.argv) > 1:
        if sys.argv[1] == "demo":
            ...
            response = chat_with_beeai_platform(message)
            print(response)
        else:
